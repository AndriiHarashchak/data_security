...
class MD5Hasher {
  Uint32List _digest = new Uint32List.fromList([0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476]);
  List<int> _s = [
  ...
  Int32List _K = new Int32List.fromList([
  ...
  String generateMD5Hash(String data) {
    _digest = new Uint32List.fromList([0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476]);
  ...
  Future<String> generateMD5HashFromFile(String path) async {
    ...
    while (!isEnd) {
      ...
      if (data.length < 64) {
        ...
        var normalizedData = _normalizeLenght(Uint8List.fromList(Uint8List.fromList(data)), false, lenghtInBytes: lenght);
        _generatehash(Uint8List.fromList(normalizedData));
      } else {
  ...
  Uint8List _normalizeLenght(Uint8List data, bool isSingleBlock, {int lenghtInBytes = 0}) {
    var addLength = (56 - ((data.length + 1) % 64)) % 64; //new lenght with padding
    var processedInput = new Uint8List(data.length + 1 + addLength + 8); //
  ...
  void _generatehash(Uint8List chunk) {
    ...
    for (int i = 0; i < chunk.length / 64; ++i) {
      // copy the input to M
      Uint32List M = new Uint32List(16);
      for (int j = 0; j < 16; ++j) {
        int index = (i * 64) + (j * 4);
        M[j] = ByteData.view(Uint8List.fromList([chunk[index], chunk[index + 1], chunk[index + 2], chunk[index + 3]]).buffer).getUint32(0, Endian.little);
      }
      //M[j] = ].asByteArray.ge
      // initialize round variables
      var A = _digest[0], B = _digest[1], C = _digest[2], D = _digest[3], e = 0, f = 0;
  ...
  static Uint8List int64LitteEndianBytes(int value) => Uint8List(8)..buffer.asByteData().setInt64(0, value, Endian.little);
  static Uint8List int32LitteEndianBytes(int value) => Uint8List(4)..buffer.asByteData().setInt32(0, value, Endian.little);

  String _hexEncode(Uint32List bytes) {
    String output = "";
    bytes.forEach((element) {
      var bytes = int32LitteEndianBytes(element);
      bytes.forEach((element2) {
